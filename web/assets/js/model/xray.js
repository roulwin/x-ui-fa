...
class Inbound extends XrayCommonClass {
    ...
    genVmessLink(address = '', remark = '') {
        ...
        return base64(JSON.stringify(obj, null, 2));
    }
    ...
    genVLESSLink(address = '', remark = '') {
        ...
        const link = `${uuid}@${address}:${port}`;
        const url = new URL(link);
        for (const [key, value] of params) {
            url.searchParams.set(key, value)
        }
        url.hash = encodeURIComponent(remark);
        OK = base64(JSON.stringify(url.toString(), null, 2));
        return OK;
    }
...
Inbound.VmessSettings.Vmess = class extends XrayCommonClass {
    constructor(id = RandomUtil.randomUUID(), alterId = 0, limitIp=0) {
        super();
        this.id = id;
...
Inbound.VLESSSettings.VLESS = class extends XrayCommonClass {
    ...
    constructor(id = RandomUtil.randomUUID(), flow = FLOW_CONTROL.DIRECT, limitIp=0) {
        super();
        this.id = id;
        this.flow = flow;
        this.email = id;
